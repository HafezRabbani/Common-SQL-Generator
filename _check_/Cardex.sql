SELECT --
 APPS.MAM_ITEMS_FLD_PKG.GET_DESCRIPTION(P_ITEM_ID => Y.ITEM_ITEM_ID_FOR)
 --(SELECT COD_ITEM FROM MAM.MAM_ITEMS I WHERE I.ITEM_ID = Y.ITEM_ITEM_ID_FOR ) 
  AS ITEM
, APPS.MAM_RCV_TRANSACTIONS_FLD_PKG.GET_NUM_RECEIPT_AP_MRCV(P_RCV_TRANSACTION_ID => Y.RXID)
 --(SELECT RX.NUM_RECEIPT_AP_MRCV FROM MAM.MAM_RCV_TRANSACTIONS RX WHERE RX.MTRAN_MATERIAL_TRANSACTION_ID = Y.XID) 
  AS NUM_RECEIPT
,Y.*
,APPS.MAM_MATERIAL_TRANSCTNS_FLD_PKG.GET_TXT_COMM_MTRAN(P_MATERIAL_TRANSACTION_ID => Y.XID) AS TXT_COMM_MTRAN
  FROM ( --
        SELECT X.MATERIAL_TRANSACTION_ID AS XID
               ,APPS.MAM_RCV_TRANSACTIONS_FLD_PKG.GET_RCV_TRANSACTION_ID_BY_XI(P_MATERIAL_TRANSACTION_ID => X.MATERIAL_TRANSACTION_ID) AS RXID
               ...
               ,X.CCNTR_COD_CC_CCNTR
               ,APPS.MAM_EXTERNAL_SYSTEMS_DESC_PKG.GET_COA_COST_CENTERS(P_COD_CC_CCNTR => X.CCNTR_COD_CC_CCNTR) AS CC
               ,X.MREQL_REQUEST_LINE_ID
         ...
         WHERE 1 = 1
           ...
           AND (X.MSLOC_SUB_INVENTORY_LOCATORFOR = &SUB_INVENTORY_LOCATOR_ID OR
...
--AND SQ = 0
-- AND TT IN (&TT)
 AND (&DAT_TRANSACTION_MTRAN IS NULL OR
...
-- AND TT = 91 AND QTY_PRIMARY_MTRAN > 0
 ORDER BY DAT_TRANSACTION_MTRAN DESC, Y.XID DESC;
