SELECT CASE
      ...
      ,(SELECT COUNT(1)
          FROM MAM.MAM_PHYSICAL_ADJUSTMENTS J
         INNER JOIN MAM.MAM_PHYSICAL_SUBINVENTORIES S
            ON J.MPSUB_PHYSICAL_SUBINVENTORY_ID =
               S.PHYSICAL_SUBINVENTORY_ID
         INNER JOIN MAM.MAM_PHYSICAL_INVENTORIES P
            ON S.MPINV_PHYSICAL_INVENTORY_ID = P.PHYSICAL_INVENTORY_ID
         WHERE P.PHYSICAL_INVENTORY_ID = PI.PHYSICAL_INVENTORY_ID) AS COUNT_ADJ
      ,(SELECT COUNT(1)
          FROM MAM.MAM_MATERIAL_TRANSACTIONS X
         INNER JOIN MAM.MAM_PHYSICAL_ADJUSTMENTS J
            ON X.MPADJ_PHYSICAL_ADJUSTMENT_ID = J.PHYSICAL_ADJUSTMENT_ID
         INNER JOIN MAM.MAM_PHYSICAL_SUBINVENTORIES S
            ON J.MPSUB_PHYSICAL_SUBINVENTORY_ID =
               S.PHYSICAL_SUBINVENTORY_ID
         INNER JOIN MAM.MAM_PHYSICAL_INVENTORIES P
            ON S.MPINV_PHYSICAL_INVENTORY_ID = P.PHYSICAL_INVENTORY_ID
         WHERE P.PHYSICAL_INVENTORY_ID = PI.PHYSICAL_INVENTORY_ID) AS COUNT_X71
      ,(SELECT COUNT(1)
