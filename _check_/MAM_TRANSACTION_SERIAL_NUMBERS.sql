SELECT (SELECT I.COD_ITEM || ': ' || I.DES_ITEM
      ...
      ,APPS.MAM_SERIAL_NUMBERS_FLD_PKG.GET_DESCRIPTION(P_SERIAL_NUMBER_ID => XS.MSER_SERIAL_NUMBER_ID) AS SERIAL
      ...
      ,(SELECT FA.NUM_ASSET_FIXAS
          FROM MAM.MAM_MATERIAL_TRANSACTIONS X
         INNER JOIN MAM.MAM_TRANSACTION_SERIAL_NUMBERS T
            ON X.MATERIAL_TRANSACTION_ID = T.MTRAN_MATERIAL_TRANSACTION_ID
         INNER JOIN FIA.FIA_FIXED_ASSETS FA
            ON T.MSER_SERIAL_NUMBER_ID = FA.MSER_SERIAL_NUMBER_ID
         WHERE X.ID_TRANSFER_MTRAN = XS.MTRAN_MATERIAL_TRANSACTION_ID) AS NUM_ASSET_FIXAS_RPLC
      ,CASE
         WHEN EXISTS
          (SELECT NULL
                 FROM FIA.FIA_FIXED_ASSETS F
                WHERE F.MSER_SERIAL_NUMBER_ID = XS.MSER_SERIAL_NUMBER_ID
                  AND F.MTRAN_MATERIAL_TRANSACTION_ID =
                      XS.MTRAN_MATERIAL_TRANSACTION_ID) THEN
          'OK'
       END AS EXISTS_ON_FIXED_ASSETS
      ,(SELECT RX.NUM_RECEIPT_AP_MRCV
          FROM MAM.MAM_RCV_TRANSACTIONS RX
         WHERE RX.MTRAN_MATERIAL_TRANSACTION_ID =
               XS.MTRAN_MATERIAL_TRANSACTION_ID) AS NUM_RECEIPT_AP_MRCV
      ,XS.*
  FROM --DELETE   --
      ...
      */
      /*    
       ...
       (SELECT NULL
               ...
               INNER JOIN MAM.MAM_MATERIAL_TRANSACTIONS X2
                  ON I2.ITEM_ID = X2.ITEM_ITEM_ID_FOR
               WHERE X2.MATERIAL_TRANSACTION_ID = XS.MTRAN_MATERIAL_TRANSACTION_ID
                 AND I2.COD_ITEM IN (&COD_ITEM))
      */
   AND XS.CREATE_DATE > TRUNC(SYSDATE)
--   FOR UPDATE
