SELECT (SELECT 1
 ...
 WHERE 1 = 1
      ...
      --                    
       AND (FA.MSER_SERIAL_NUMBER_ID IN (&SERIAL_NUMBER_ID) OR
...
SELECT (SELECT I.COD_ITEM
      ...
      ,APPS.APP_FND_LOOKUP_PKG.GET_FARSI_MEANING_FUN(UPPER('MAM_SERIAL_NUMBERS'), UPPER('LKP_STA_CURRENT_MSER'), S.LKP_STA_CURRENT_MSER) AS LKP_STA_CURRENT_MSER_DES
      ,(SELECT 'USED'
          FROM DUAL
         WHERE EXISTS
         (SELECT NULL
      ...
      ,S.DAT_DISABLE_MSER
      ,TO_CHAR(S.DAT_DISABLE_MSER, 'YYYY/MM/DD HH24:MI:SS', 'NLS_CALENDAR=PERSIAN') AS DAT_DISABLE_MSER_H
       ...
       (EXISTS
        (SELECT NULL
          ...
          WHERE I.ITEM_ID = S.ITEM_ITEM_ID
                AND I.COD_ITEM IN (&COD_ITEM)) OR &COD_ITEM IS NULL) --
      --                    
       AND (S.SERIAL_NUMBER_ID IN (&SERIAL_NUMBER_ID) OR
       &SERIAL_NUMBER_ID IS NULL) --
   FOR UPDATE
--
;
