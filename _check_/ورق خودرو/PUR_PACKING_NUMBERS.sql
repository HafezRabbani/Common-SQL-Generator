SELECT PN.PACKING_NUMBER_ID
      ...
      ,(SELECT DISTINCT RX.ITEM_ITEM_ID
          FROM MAM.MAM_RCV_TRANSACTIONS RX
         WHERE RX.PAKNR_PACKING_NUMBER_ID = PN.PACKING_NUMBER_ID) || ' - ' ||
       APPS.MAM_ITEMS_CTRL_PKG.GET_DESCRIPTION(P_ITEM_ID => (SELECT DISTINCT RX.ITEM_ITEM_ID
                                                               FROM MAM.MAM_RCV_TRANSACTIONS RX
                                                              WHERE RX.PAKNR_PACKING_NUMBER_ID =
                                                                    PN.PACKING_NUMBER_ID)) AS ITEM
      ,(SELECT DISTINCT RX.NUM_RECEIPT_AP_MRCV
          FROM MAM.MAM_RCV_TRANSACTIONS RX
         WHERE RX.PAKNR_PACKING_NUMBER_ID = PN.PACKING_NUMBER_ID) AS NUM_RECEIPT_AP_MRCV
      ...
      ,PN.CREATE_DATE
      ,TO_CHAR(PN.CREATE_DATE
              ,'yyyy/mm/dd hh24:mi:ss'
              ,'nls_calendar=persian') AS CREATE_DATE_H
      ,PN.CREATE_BY_APP_USER
      ,(SELECT U.NAM_USER_USRS
          FROM FND.FND_USERS U
         WHERE U.IDE_USER_USRS = PN.CREATE_BY_APP_USER) AS USER_
  FROM --
...
--   AND PN.NUM_EXT_COIL_PAKNR IN (&NUM_EXT_COIL_PAKNR)
 ...
 (SELECT NULL
          FROM MAM.MAM_RCV_TRANSACTIONS RX
         WHERE RX.PAKNR_PACKING_NUMBER_ID = PN.PACKING_NUMBER_ID
           AND RX.NUM_RECEIPT_AP_MRCV = &NUM_RECEIPT_AP_MRCV)
   FOR UPDATE
